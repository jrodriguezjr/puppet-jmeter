...
class jmeter(
  ...
  if $jmeter_plugins_install == True {
    ...
    exec { 'install-jmeter-plugins':
      command => "unzip -q -d JMeterPlugins JMeterPlugins-Standard-${jmeter_plugins_version}.zip && mv JMeterPlugins/lib/ext/JMeterPlugins-Standard.jar /usr/share/jmeter/lib/ext && rm -rf /root/JMeterPlugins",
      cwd     => '/root',
